...
export default function PurchaseInvoice() {
  ...
  async function loadInvoiceForEdit(purchaseId) {
    try {
      ...
      // fn_get_purchase_invoice(p_purchase_id int)
      // returns: { header: {...}, items: [...] }
      const { data, error } = await supabase.rpc(
        "fn_get_purchase_invoice",
      ...
      // Lines
      // ⚠️ Ensure your fn_get_purchase_invoice returns fields that match below
  ...
  async function addLine() {
    ...
    try {
      const { data, error } = await supabase.rpc("fn_lookup_product_by_id", {
        p_company_id: companyId,
        p_state_id: stateId,
        p_branch_id: branchId,
  ...
  async function handleSave() {
    ...
    try {
      let rpcName = "fn_create_purchase_invoice";
      let rpcArgs = { p_data: payload };

      // If editing, call update RPC instead
      if (currentPurchaseId) {
        // ⚠️ Implement fn_update_purchase_invoice(p_purchase_id int, p_data jsonb)
        rpcName = "fn_update_purchase_invoice";
